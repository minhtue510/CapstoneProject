...
const OrderDetailScreen = ({ route }) => {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => (
        <TouchableOpacity onPress={() => navigation.navigate('Home')} style={OrderDetailScreenStyles.backButton}>
  ...
  const changeOrderStatus = async () => {
    if (!image) {
      Alert.alert('Thông báo', 'Vui lòng chọn hoặc chụp ảnh trước khi hoàn thành đơn hàng.');
      return;
    }
          ...
          },
          {
            ...
            onPress: async () => {
              try {
                ...
                setIsOrderDelivered(updatedOrderInfo?.itemOrderTripResponse?.orderTrip?.status === 4);

                // Thông báo hoàn thành
                if (updatedOrderInfo?.itemOrderTripResponse?.orderTrip?.status === 4) {
                ...
                Alert.alert('Lỗi', 'Không thể cập nhật thông tin đơn hàng. Vui lòng thử lại sau.');
              }
            },
          },
        ],
  ...
  return (
    <View style={OrderDetailScreenStyles.container}>
      <View style={OrderDetailScreenStyles.section}>
        ...
        {orderInfo.itemOrderTripResponse && orderInfo.itemOrderTripResponse.map((e, index) => (
          <View style={OrderDetailScreenStyles.section} key={index}>
            ...
            {e.orderTrip?.type === 1 ? (
              <>
                ...
                <Text style={OrderDetailScreenStyles.sectionText}>Số điện thoại người gửi: {orderInfo.orderLocation?.getPhone}</Text>
                <Text style={OrderDetailScreenStyles.sectionText}>Địa chỉ nhận: {orderInfo.orderLocation?.addressGet}, {orderInfo.orderLocation?.cityGet}, {orderInfo.orderLocation?.provinceGet}</Text>
              </>
            ) : (
              <>
                ...
                <Text style={OrderDetailScreenStyles.sectionText}>Địa chỉ giao: {orderInfo.orderLocation?.addressDelivery}, {orderInfo.orderLocation?.cityDelivery}, {orderInfo.orderLocation?.provinceDelivery}</Text>
              </>
            )}
            <TouchableOpacity onPress={() => navigation.navigate('MapScreen', {
              getLongitude: orderInfo.orderLocation.longtitudeGet,
              getLatitude: orderInfo.orderLocation.lattitudeGet,
              deliveryLongitude: orderInfo.orderLocation.longtitudeDelivery,
      ...
      <View style={OrderDetailScreenStyles.containerIcon}>
        {!isOrderDelivered && orderInfo.itemOrderTripResponse?.orderTrip?.status !== 4 && (
          <TouchableOpacity
        ...
        {image && (
          <TouchableOpacity onPress={openModal}>
            <Image source={{ uri: image }} style={OrderDetailScreenStyles.image} />
          </TouchableOpacity>
        )}
      </View>
